...
(define (obstacle_blocking?)
  (printf "\ntarget degrees to rotate:~a\nobstacle_degrees_to_rotate:~a\ndistance_to_obstacle:~a\ndistance_to_target:~a\nlol\n" target_degrees_to_rotate obstacle_degrees_to_rotate distance_to_obstacle distance_to_target)
  (cond
    ((and (negative? target_degrees_to_rotate)
          (< (+ target_degrees_to_rotate -15) obstacle_degrees_to_rotate (- target_degrees_to_rotate -15))
...
(define evasion_rotation 0)
(define (evade)
  (cond
    ((negative? obstacle_degrees_to_rotate) ;negative go right of obstacle
     (set! evasion_rotation 15)
     (set! obstacle_degrees_to_rotate (+ obstacle_degrees_to_rotate evasion_rotation))
     (rotate_to_face obstacle_degrees_to_rotate)
     (setMotors -145 -150)
     (sleep (time_to_cover_distance_to_obstacle))
     (setMotors 0 0)
     (post_evade))
    (else ;positive go left of obstacle
     (set! evasion_rotation -15) 
     (set! obstacle_degrees_to_rotate (+ obstacle_degrees_to_rotate evasion_rotation))
     (rotate_to_face obstacle_degrees_to_rotate)
     (setMotors -145 -150)
...
(define (post_evade)
  ...
  (printf "post_evade_angle_to_target~a\nobstacle_degrees_to_rotate~a\n" post_evade_angle_to_target obstacle_degrees_to_rotate)
  (cond
    ((negative? evasion_rotation)
     (rotate_to_face post_evade_angle_to_target)
     (printf "NOT -1\n"))
    ((positive? evasion_rotation)
     (rotate_to_face (* -1 post_evade_angle_to_target))
     (printf "-1 ALL ALONG\n")))
  (setMotors -145 -150)
...
(define (finder)
  ...
  (set! obstacle_locator (index-of (map (lambda (x) (and (> (color-green x) 130)
                                                         (< (color-blue x) 80)
                                                         (< (color-red x) 20))) processed_img) #t))
  (cond
    ((number? target_locator) (set! rotate_counter 0))
    (else
     (cond
       ((> rotate_counter 6)
        (setLCDMessage "Objective Absent" 1)
        (exit 1)))
     (rotate_62.2)
...
(define (target_distance_helper x y z) ;where x/y/z= color-green/color-red/color-blue
  (for ([i (in-range (+ target_ycord 1) 480)]
        #:break (< (color-red (get_pixel target_xcord i img)) 180))
    (cond
      ((and
  ...
  (printf "target_ycord:~a\n" target_ycord)
  (printf "target_ycord_end:~a\n" target_ycord_end)
...
(define (obstacle_distance_helper x y z) ;where x/y/z= color-green/color-red/color-blue
  (for ([i (in-range (+ obstacle_ycord 1) 480)]
        #:break (< (x (get_pixel obstacle_xcord i img)) 120))
    (cond
      ((and
        (> (x (get_pixel obstacle_xcord i img)) 120)
        (< (y (get_pixel obstacle_xcord i img)) 100)
        (< (z (get_pixel obstacle_xcord i img)) 30))
       (set! obstacle_ycord_end i))
      ))
  (printf "obstacle_ycord:~a\n" obstacle_ycord)
  (printf "obstacle_ycord_end:~a\n" obstacle_ycord_end)
  (set! obstacle_in_image_height (- obstacle_ycord_end obstacle_ycord))
  (set! distance_to_obstacle (distance_to_something obstacle_height obstacle_in_image_height)))
...
(define (main)
  ...
  (cond
    ...
    (else
     ...
     (setMotors -145 -150)
     (printf "time_to_cover_distance_to_target:~a/n" (time_to_cover_distance_to_target))
